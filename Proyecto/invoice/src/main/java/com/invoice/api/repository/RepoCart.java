...
public interface RepoCart extends JpaRepository<Cart, Integer>{
	...
	Cart findByGtinAndStatus(String gtin, Integer status);

	@Modifying
	@Transactional
	@Query(value ="UPDATE cart SET quantity = :stock WHERE cart_id = :cart_id", nativeQuery = true)
	Integer updateStock(@Param("cart_id") Integer cart_id, @Param("stock") Integer stock);
	...
	@Modifying
	@Transactional
	@Query(value ="DELETE FROM cart WHERE status = 0", nativeQuery = true)
	void deleteCart();
}
